...
export class TasksComponent {
  ...
  values = signal<TasksGet>({
    page: 1,
    page_size: 12,
    sort_by: 'due_date',
    sort_type: 'asc',
  });
  taskCreate = signal<TaskCreateValues>({
    title: null,
    description: null,
    completed: true,
    due_date: null,
  });
  taskCreateErrors = signal<TaskCreateErrors>({
    title: null,
    description: null,
    completed: null,
    due_date: null,
  ...
  isErrors = computed(
    () =>
      this.taskCreateErrors().title ||
      this.taskCreateErrors().description ||
      this.taskCreateErrors().completed ||
      this.taskCreateErrors().due_date ||
      this.taskCreateErrors().global
  );
  ...
  deleteTask(id: number, e: MouseEvent) { 
    e.stopPropagation();
    this.loadingState.set(true);

    this.http
      .delete<Task>(`${BACKEND_URL}api/task/${id}`, {
        headers: {
      ...
      .subscribe((res: Task) => {
        if (res) {
          ...
          this.values.update((prev) => ({ ...prev, page: 1 }));
        }
      });
  }
  ...
  changeField(event: Event, type: 'title' | 'description' | 'due_date') {
    ...
    this.taskCreate.update((prev) => ({
      ...prev,
      [type]: value ? value : null,
    }));
    this.taskCreateErrors.update((prev) => ({
      ...prev,
      [type]: null,
      global: null,
    }));
  ...
  handleSubmit() {
    ...
    if (data.title && data.description && !this.isErrors()) {
      ...
      this.http
        .post<Task>(`${BACKEND_URL}api/task/`, this.taskCreate(), {
          headers: {
            Authorization: `Bearer ${localStorage.getItem('access_token')}`,
          },
        })
